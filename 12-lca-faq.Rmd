...
warnings_table_data <- extract_warnings(final_data)

# warnings_table_data is assumed to exist
library(gt)
...
if (nrow(warnings_table_data) > 0) {
 ...
 # Save table as an image (PNG) in the figures/ folder
  gtsave(
    data = warnings_table,
    filename = "warnings_table.png",
    path = "figures"
  )
} else {
  cat("No warnings detected in any output files.\n")
}
...
```{r}
source(here("functions", "plot_lca.txt"))
