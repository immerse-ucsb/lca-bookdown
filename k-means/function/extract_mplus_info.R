...
extract_mplus_info_extended <- function(file_path) {
  ...
  #### 8. ERROR EXTRACTION ####
  error_keywords <- c("NON-POSITIVE DEFINITE", "SADDLE", "SINGULARITY")  # ✅ Added "SINGULARITY"
  
  # Extract all lines containing error messages
  error_lines <- grep(paste(error_keywords, collapse = "|"), output, ignore.case = TRUE)
  
  # If errors exist, capture additional context (2 lines after the match)
  error_messages <- list()
  if (length(error_lines) > 0) {
    for (line_idx in error_lines) {
      # ✅ **Backtrack to find the REAL start of the error message**
      start_idx <- line_idx
      while (start_idx > 1 && !grepl("^\\s*$", output[start_idx - 1])) {  
        start_idx <- start_idx - 1
      }
      
      # ✅ **Capture a few lines after for full context**
      end_idx <- min(line_idx + 5, length(output))  # Captures up to 5 additional lines
      error_block <- output[start_idx:end_idx]
      
      # ✅ **Clean and store message**
      full_message <- paste(error_block, collapse = " ")
      error_messages <- append(error_messages, full_message)
    }
  }
  
  # ✅ Store errors in a tibble, ensuring structure
  errors <- if (length(error_messages) > 0) {
    tibble(Error_Message = unique(error_messages))  # ✅ Prevent duplicate errors
  } else {
    tibble(Error_Message = NA_character_)  # ✅ Ensures consistent structure
  }
  
  # ✅ **Count number of errors**
  error_count <- if (length(error_messages) > 0) length(unique(error_messages)) else 0
